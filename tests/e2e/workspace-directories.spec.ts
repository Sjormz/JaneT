...
test('sets up a main directory and restores independent linked-folder sessions', async ({}, testInfo) => {
  ...
  try {
    ...
    await expect(firstTerminal.locator('.xterm-helper-textarea')).toBeFocused();
    await page.keyboard.type('echo JANET_FOCUS_RETURNED');
    await page.keyboard.press('Enter');
    await expect(firstTerminal.locator('.xterm-rows')).toContainText(/JANET_FOCUS_RETURNED.*JANET_FOCUS_RETURNED/s);
    ...
    await expect(firstTerminal.locator('.xterm-helper-textarea')).toBeFocused();
    await page.getByRole('button', { name: 'Expand project tools' }).focus();
    await page.keyboard.press('Tab');
    await expect(firstTerminal.locator('.xterm-helper-textarea')).not.toBeFocused();
    await page.locator('.workspace-group-toggle').first().click();
    await expect(firstTerminal.locator('.xterm-helper-textarea')).toBeFocused();
    await page.locator('.workspace-group-toggle').first().click();
    await expect(firstTerminal.locator('.xterm-helper-textarea')).toBeFocused();
