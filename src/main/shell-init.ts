...
export function buildShellInit(shell: string): string {
  ...
  if (base === 'powershell' || base === 'powershell.exe' || base === 'pwsh' || base === 'pwsh.exe') {
    ...
    const ps = [
      ...
      "          $commandName = $commands[-1].GetCommandName()",
      "          $commandInfo = if ($commandName) { Get-Command -Name $commandName -ErrorAction SilentlyContinue }",
      "          while ($commandInfo -and $commandInfo.CommandType -eq 'Alias') { $commandInfo = $commandInfo.ResolvedCommand }",
      "          $global:__jt_state.UseNativeExitCode = $commandInfo -and $commandInfo.CommandType -eq 'Application'",
      "        }",
      "      } catch {}",
      ...
      "    $commandStatus = if ($__jt_success) { 0 } elseif ($global:__jt_state.UseNativeExitCode -and $null -ne $__jt_last_exit_code) { $__jt_last_exit_code } else { 1 }",
