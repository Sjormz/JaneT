...
export function buildShellInit(shell: string): string {
  ...
  if (base === 'powershell' || base === 'powershell.exe' || base === 'pwsh' || base === 'pwsh.exe') {
    ...
    const ps = [
      ...
      "        $commands = if ($lastStatement -is [System.Management.Automation.Language.PipelineAst]) { @($lastStatement.FindAll({ param($node) $node -is [System.Management.Automation.Language.CommandAst] }, $true)) } else { @() }",
